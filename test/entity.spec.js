    ...
    context('monolith with entity and dto suffixes', () => {
      describe('with entity and dto suffixes', () => {
        ...
        it('correctly writes the repository', () => {
          assert.fileContent(`${SERVER_MAIN_SRC_DIR}com/mycompany/myapp/repository/FooRepository.kt`, 'public interface FooRepository ');
        });

        it('correctly writes the entity', () => {
          assert.fileContent(`${SERVER_MAIN_SRC_DIR}com/mycompany/myapp/domain/FooXXX.kt`, 'public class FooXXX implements Serializable');
        });
    ...
    describe('with formated creation timestamp', () => {
      before(async () => {
        await helpers
          ...
          .withOptions({ creationTimestamp: '2016-01-20T00:00:00.000Z', withEntities: true, blueprint: 'kotlin', 'skip-ktlint-format': true })
          .withArguments(['foo'])
          .withPrompts({
            fieldAdd: false,
            relationshipAdd: false,
      ...
      describe('SQL database', () => {
        before(async () => {
          await helpers
            ...
            .withArguments(['Foo'])
            .withOptions({ regenerate: true, force: true, skipDbChangelog: true, blueprint: 'kotlin', 'skip-ktlint-format': true });
        });
