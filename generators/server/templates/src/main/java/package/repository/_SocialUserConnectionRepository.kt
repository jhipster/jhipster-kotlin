...
interface SocialUserConnectionRepository: <% if (databaseType === 'sql') { %>JpaRepository<% } else if (databaseType === 'mongodb') { %>MongoRepository<% } else if (databaseType === 'couchbase') { %>N1qlCouchbaseRepository<% } %><SocialUserConnection, <% if (databaseType === 'sql') { %>Long<% } else if (databaseType === 'mongodb' || databaseType === 'couchbase') { %>String<% } %>> {
    ...
    fun SocialUserConnection findOneByUserIdAndProviderIdAndProviderUserId(userId: String, providerId: String, providerUserId: String)

    fun deleteByUserIdAndProviderId(userId: String, providerId: String): <% if (databaseType === 'couchbase') { %>List<SocialUserConnection><% } else { %>void<% } %>

    fun deleteByUserIdAndProviderIdAndProviderUserId(userId: String, providerId: String, providerUserId: String): <% if (databaseType === 'couchbase') { %>SocialUserConnection<% } else { %>void<% } %>
}